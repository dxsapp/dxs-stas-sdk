...
- Source code is organized under `src/bitcoin`, `src/buffer`, `src/script`, and `src/transaction`, with top-level modules like `src/transaction-factory.ts`, `src/stas-bundle-factory.ts`, `src/base.ts`, and `src/hashes.ts`. (see: docs/COMMAND_LOG.md → Repo Tree (top-level + 2 depth))
- Tests live under `tests/` and include transaction and script coverage files. (see: docs/COMMAND_LOG.md → ls (after moving docs))
...
- ESLint config files are not present in the repo. (see: docs/COMMAND_LOG.md → rg --files -g "*eslint*")
- Prettier config files are not present in the repo. (see: docs/COMMAND_LOG.md → rg --files -g "*prettier*")
- Vitest and Mocha config files are not present in the repo. (see: docs/COMMAND_LOG.md → rg --files -g "*vitest*", docs/COMMAND_LOG.md → rg --files -g "*mocha*")
...
- `src/index.ts` re-exports the `bitcoin`, `buffer`, `script`, `stas-bundle-factory`, `transaction`, `transaction-factory`, `base`, and `hashes` modules. (see: src/index.ts:1-8)
...
- `ScriptToken` models opcodes and pushdata entries and can be created from raw bytes or sample tokens. (see: src/script/script-token.ts:4-65)
- `script-samples` provides sample token arrays and a canonical STAS script hex template. (see: src/script/script-samples.ts:5-26)
...
```ts
import { Address, NullDataBuilder, P2pkhBuilder, P2stasBuilder, utf8ToBytes } from "dxs-stas-sdk";
...
- `src/script/*` implements script tokenization, parsing, and builder utilities for P2PKH, P2STAS, and OP_RETURN. (see: src/script/index.ts:1-10, src/script/build/script-builder.ts:8-127)
...
- `src/transaction-factory.ts` provides high-level STAS transaction helper functions (transfer/split/merge/redeem). (see: src/transaction-factory.ts:12-221)
...
- `InputBilder.preimage` explicitly notes only `SIGHASH_ALL | FORKID` is implemented. (see: src/transaction/build/input-builder.ts:197-200)
...
- Style guidance is limited to TSLint configuration; no ESLint/Prettier configs are present. (see: tslint.json:1-3, docs/COMMAND_LOG.md → rg --files -g "*eslint*", docs/COMMAND_LOG.md → rg --files -g "*prettier*")
...
- ESLint configuration files are not present in the repo. (see: docs/COMMAND_LOG.md → rg --files -g "*eslint*")
- Prettier configuration files are not present in the repo. (see: docs/COMMAND_LOG.md → rg --files -g "*prettier*")
- Vitest/Mocha configurations are not present in the repo. (see: docs/COMMAND_LOG.md → rg --files -g "*vitest*", docs/COMMAND_LOG.md → rg --files -g "*mocha*")
